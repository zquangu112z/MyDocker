I have cloned the original Docker image from ttps://hub.docker.com/r/floydhub/dl-docker/ and modified it to support ubuntu 16.04 and python3. I also removed unuseful packages.


## All-in-one Docker image for Deep Learning
**CPU Version**
```bash
docker build -t image_name:image_tag -f Dockerfile.cpu .
```

This will build a Docker image named `image_name` and tagged depending on the tag your specify. Also note that the appropriate `Dockerfile.<architecture>` has to be used.

## Running the Docker image as a Container
**CPU Version**
```bash
docker run -it -p 8888:8888 -p 6006:6006 -v /sharedfolder:/root/sharedfolder image_name:image_tag bash
```
